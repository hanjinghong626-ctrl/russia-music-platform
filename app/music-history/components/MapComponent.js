...
const createCityIcon = () => {
  const size = 14;
  return L.divIcon({ className: 'city-marker', html: `<div style="width:${size}px;height:${size}px;background:#D4AF37;border-radius:50%;box-shadow:0 0 8px rgba(212,175,55,0.5),0 0 2px rgba(212,175,55,0.8);cursor:pointer;"></div>`, iconSize: [size,size], iconAnchor: [size/2,size/2] });
};

const createSmallCityIcon = () => {
  const size = 10;
  return L.divIcon({ className: 'city-marker small', html: `<div style="width:${size}px;height:${size}px;background:#9B8B6E;border-radius:50%;box-shadow:0 0 6px rgba(155,139,110,0.4),0 0 2px rgba(155,139,110,0.6);cursor:pointer;"></div>`, iconSize: [size,size], iconAnchor: [size/2,size/2] });
};
...
export default function MapComponent({ activePeriod, onComposerSelect, onCitySelect, mapCenter = [60, 50], mapZoom = 4 }) {
  ...
  useEffect(() => {
    ...
    style.textContent = `@keyframes marker-pulse{0%,100%{transform:scale(1);opacity:1}50%{transform:scale(1.15);opacity:.8}}@keyframes city-marker-pulse{0%,100%{transform:scale(1);box-shadow:0 0 15px rgba(212,175,55,.5),0 4px 12px rgba(0,0,0,.3)}50%{transform:scale(1.08);box-shadow:0 0 25px rgba(212,175,55,.7),0 4px 16px rgba(0,0,0,.35)}}.custom-marker{background:transparent!important;border:none!important}.city-marker{background:transparent!important;border:none!important}.leaflet-container{background:#0A0E17!important;font-family:'Noto Sans SC',sans-serif}.leaflet-control-zoom a{background:#141B2D!important;color:#D4AF37!important;border-color:rgba(212,175,55,.3)!important}.leaflet-control-zoom a:hover{background:#1E2A40!important}.leaflet-control-attribution{background:rgba(10,14,23,.8)!important;color:#6B7B8C!important;font-size:10px!important}.leaflet-control-attribution a{color:#B8C5D6!important}.marker-wrapper.dimmed>div:first-child{opacity:.12!important;box-shadow:none!important;animation:none!important}.marker-wrapper.dimmed>div:last-child{opacity:.12!important;border-color:#3a3a3a!important}.city-marker-container.small .city-marker-bg{animation:city-marker-pulse-small 2.5s ease-in-out infinite}@keyframes city-marker-pulse-small{0%,100%{transform:scale(1)}50%{transform:scale(1.1)}}`;
  ...
  const handleComposerSelectFromCard = (composerId) => { const composer = composers.find(c => c.id === composerId); if (composer && onComposerSelect) onComposerSelect(composer); setSelectedCity(null); };
  ...
  useEffect(() => {
    ...
    cities.forEach(city => {
      ...
      const cityMarker = L.marker(city.coords, { icon });
      ...
      cityMarker.bindTooltip(tc, { className: 'custom-tooltip city ' + (hasImage ? '' : 'small'), direction: 'top', offset: [0, hasImage ? -18 : -14] });
      cityMarker.on('click', () => { if (city.image) handleCitySelect(city); });
      cityMarker.addTo(map); cityMarkersRef.current.push(cityMarker);
    });
